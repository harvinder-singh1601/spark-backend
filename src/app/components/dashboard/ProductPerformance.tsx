"use client"
import { Badge, Dropdown, DropdownItem,  Select, Table, TextInput, Spinner } from "flowbite-react"
    ...
    return (
      <CardBox>
        <div className="flex items-center justify-center min-h-[400px]">
          <Spinner size="xl" />
          <span className="ml-3 text-muted">Loading products...</span>
        </div>
      </CardBox>
    );
  }
              ...
              <div className="overflow-x-auto border border-border rounded-md">
                <Table>
                  <Table.Head className="bg-gray-50" >
                    ...
                    <Table.HeadCell className="text-sm font-semibold max-w-40">
                      Product
                    </Table.HeadCell>
                    <Table.HeadCell className="text-sm font-semibold">
                      Customer
                ...
                <div className="flex items-center justify-end py-4 pt-2 px-6">
                  ...
                  <div className="flex items-center divide-x border border-border rounded-md">
                    ...
                    }} disabled={(currentpagination === 1)} className={`py-1.5 px-4 text-sm font-medium hover:bg-lightprimary hover:text-primary ${currentpagination === 1 ? 'bg-gray-100 cursor-not-allowed text-gray-300 hover:!bg-gray-100 hover:!text-gray-300':'cursor-pointer hover:bg-none hover:text-primary'}`}>Previous</button>
                    {Array.from({ length: Math.ceil(fixedProducts.length/itemsCount) }).map((_,index) => (
                  <button key={index} className={`py-1.5 px-4 text-sm font-medium hover:bg-lightprimary hover:text-primary cursor-pointer ${currentpagination === (index+1) ? 'bg-lightprimary text-primary':null}`}>{index+1}</button>
                    ))}
                    <button onClick={() => {
                      setCurrentPagination((prev) => {
